...
class MasterController extends BaseController
{
     ...
     * -------------------------------------------------------
     */
    public function __construct(Category $category,User $user,Notification $notification,UserRoleMaster $userRoleMaster,Product $product, Banner $banner)
    {
        parent::__construct();
    ...
    public function notificationList(Request $request) 
    {
        ...
        if ( $validator->fails() ) {
            ...
            $notificationObj = $notificationObjData->paginate(20); /** Pagination */
    ...
    public function productList(Request $request)
    {
        try{
            ...
            if ( $validator->fails() ) {
                ...
                $query = $this->product::with('category','productMedia')->whereStatus(config('constant.status_active_value'))->whereNull('deleted_at');
                if($request->has('category_id')){
                    $productObjData = $query->where('category_id',$request->category_id);
                }
                $productObjData = $query->orderBy('created_at', 'DESC');
                $productObj = $productObjData->paginate(config('constant.job_page_limit')); /** Pagination */
                $products = $productObj;
                $productFinalArr =ProductResource::collection($products);
                $productArre['products'] = $productFinalArr;
                $lastPage = $productObj->lastPage(); /** Last page of record */
                if ($currentPage > $lastPage) { /** Check if this is last page or not */
    ...
    {
        try{
            $validator = Validator::make($request->all(), [ 'id' => 'required' ]); /** Validation code */
            if ( $validator->fails() ) {
                throw new Exception($validator->errors()->first(),400);
            }
